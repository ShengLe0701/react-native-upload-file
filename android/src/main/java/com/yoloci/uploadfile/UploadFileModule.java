    ...
    public void upload(final ReadableMap options, final Callback callback) {
        ...
        try {
            ...
            for (int i = 0; i < files.size(); i++) {
                ...
                outputStream.writeBytes(twoHyphens + boundary + lineEnd);
                outputStream.writeBytes("Content-Disposition: form-data; name=\"" + ( name != null && name != "" ? name : filename ) +"\";filename=\"" + filename + "\"" + lineEnd);
                outputStream.writeBytes(lineEnd);
            ...
            callback.invoke(serverResponseCode, serverResponseMessage, map);


        } catch(Exception ex) {
            callback.invoke(401, ex.getMessage(), null);
        }
    }
}
